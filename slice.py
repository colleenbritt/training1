...
class DataSlice(object):
    ...
    def csv(self):
        '''
        Gets and formats info from the csv file
        '''
...
def csv_next_slice():
    '''
    Gets next slice of info
    '''
    for line in generator():
        s_l = DataSlice(tuple(line[0:3]), tuple(line[3:6]), tuple(line[6:9]), tuple(line[9:12]), line[12], tuple(line[13:16]), tuple(line[16:19]), line[19], line[20], tuple(line[21:28]), line[28], line[29], line[30])
        yield s_l

for s_l in csv_next_slice():
    print(s_l.csv())
